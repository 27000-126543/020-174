...
export function ReviewForm() {
  ...
  const autoGeneratedSpeeches = useMemo(() => {
    if (isSmooth === false && allStuckPoints.length > 0) {
      const speeches: { content: string; stuckPoint: string }[] = [];
      allStuckPoints.forEach((point) => {
        const mapped = autoSpeechMap[point];
        if (mapped && mapped.length > 0) {
          speeches.push({ content: mapped[0], stuckPoint: point });
        }
      });
      return speeches;
    }
    ...
    if (isSmooth === false && autoGeneratedSpeeches.length > 0) {
      autoGeneratedSpeeches.forEach((auto) => {
        addPersonalSpeech({
          ...
          tags: ['objection_handling', auto.stuckPoint, '自动沉淀'],
        });
      });
    }
                ...
                {autoGeneratedSpeeches.map((auto, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs px-2 py-0.5 bg-amber-200 text-amber-700 rounded-md">
                        {auto.stuckPoint}
                      </span>
                      <span className="text-xs text-amber-500">自动沉淀</span>
                    </div>
